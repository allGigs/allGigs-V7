...
AUTOMATION_DETAILS_PATH = IMPORTANT_DIR / 'automation_details.csv'
...
def freelance_directory(files_read, company_name):
    ...
    try:
        ...
        for std_col, src_col in mapping.items():
            if src_col in files_read.columns:
                result[std_col] = files_read[src_col]
            else:
                result[std_col] = src_col
...
def main():
    ...
    try:
        ...
        # Load automation details list
        try:
            automation_details = pd.read_csv(AUTOMATION_DETAILS_PATH, sep=';')
            logging.info(f"Loaded automation details with {len(automation_details)} sources")
        except Exception as e:
            msg = f"FAILED: automation details - {e}"
            logging.error(msg)
            error_messages.append(msg)
            source_failures.append(("automation details", str(e)))
            return
        
        # Get existing historical data first
        try:
        ...
        for index, row in automation_details.iterrows():
            try:
                ...
                for separator in [',', ';', '\t']:
                    try:
                        files_read = pd.read_csv(url_link, sep=separator).fillna('')
                ...
                if files_read is None:
                    msg = f"FAILED: {company_name} - Could not read CSV ({url_link})"
                    logging.error(msg)
                    error_messages.append(msg)
                    broken_urls.append((company_name, url_link))
                    source_failures.append((company_name, f"Could not read CSV ({url_link})"))
                ...
                if not company_df.empty:
                    ...
                    msg = f"FAILED: {company_name} - DataFrame is empty after cleaning"
                    logging.error(msg)
                    error_messages.append(msg)
                    source_failures.append((company_name, "DataFrame is empty after cleaning"))
                # Add an empty line after each company
                logging.info("")
