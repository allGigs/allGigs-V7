...
def prepare_data_for_upload(df, historical_data=None):
    ...
    # Add true_duplicates analysis
    logging.info("=" * 80)
    logging.info("TRUE DUPLICATES ANALYSIS")
    logging.info("=" * 80)
    
    # Group by true_duplicates and count rows per group
    true_dup_counts = df.groupby('true_duplicates').size().reset_index(name='count')
    
    # Find groups with multiple entries (true duplicates)
    true_duplicate_groups = true_dup_counts[true_dup_counts['count'] > 1]
    
    if not true_duplicate_groups.empty:
        logging.info(f"Found {len(true_duplicate_groups)} groups with true duplicates (same source + title + skills):")
        
        # Get detailed info for true duplicate groups
        for _, group_info in true_duplicate_groups.head(10).iterrows():  # Show first 10 groups
            true_dup_id = group_info['true_duplicates']
            count = group_info['count']
            
            # Get all jobs in this group
            group_jobs = df[df['true_duplicates'] == true_dup_id]
            unique_titles = group_jobs['Title'].unique()
            unique_sources = group_jobs['Source'].unique() if 'Source' in group_jobs.columns else group_jobs['Company'].unique()
            unique_companies = group_jobs['Company'].unique()
            unique_urls = group_jobs['URL'].unique()
            
            logging.info(f"true_duplicates: {true_dup_id[:12]}... ({count} jobs)")
            logging.info(f"  Title: {unique_titles[0]}")  # All titles should be the same
            logging.info(f"  Sources: {list(unique_sources)}")
            logging.info(f"  Companies: {list(unique_companies)}")
            logging.info(f"  URLs: {list(unique_urls[:3])}{'...' if len(unique_urls) > 3 else ''}")  # Show first 3 URLs
            logging.info("-" * 40)
        
        if len(true_duplicate_groups) > 10:
            logging.info(f"... and {len(true_duplicate_groups) - 10} more groups")
    else:
        logging.info("No true duplicates found - all jobs are unique within their source")
